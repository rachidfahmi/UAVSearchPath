...
public:
    UAVSearchCell(const std::vector<int>& id,
        ...
        : GridCell<UAVSearchState, double>(id, config), alpha(0.05), cellId(id) {
    ...
    [[nodiscard]] UAVSearchState localComputation(
        ...
        const std::unordered_map<std::vector<int>,
            NeighborData<UAVSearchState, double>>& neighborhood) const override {

        UAVSearchState next = state;

        // ── State 200: locked on target — never moves again ───────────────
        if (state.uav == 200) {
            next.uav = 200;
        }

        // ── Step 1: UAV present → decide move or lock ─────────────────────
        else if (state.uav == 100) {
            double bestNeighborProb = 0.0;
            int bestDir = 1;
            for (int d = 1; d <= 8; d++) {
            ...
            if (bestNeighborProb <= 0.0) {
                // flat field — no gradient yet, wait for diffusion to arrive
                next.uav = 100;
            } else if (state.prob >= bestNeighborProb) {
                // at local maximum with detectable gradient — lock
                next.uav = 200;
            } else {
                // gradient detected — move toward best neighbor
                next.uav = bestDir;
            }
        }

        // ── Step 2: empty cell → check if a neighbor points here ─────────
        else if (state.uav == 0) {
        ...
        // ── Step 3: stale direction code → clear ─────────────────────────
        else if (state.uav >= 1 && state.uav <= 8) {
            next.uav = 0;
        }

        // ── Diffusion (always, every cell, every tick) ────────────────────
